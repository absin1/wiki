...
public class FetchOnlineDataExample {
	...
	public static void main(String[] args) throws MediaWikiApiErrorException {
		...
		if (q42 instanceof ItemDocument) {
			System.out.println(
					"The English name for entity Q42 is " + ((ItemDocument) q42).getLabels().get("en").getText());
		}
		...
		for (EntityDocument ed : results.values()) {
			System.out.println("Successfully retrieved data for " + ed.getEntityId().getId());
		}
		...
		if (q8 instanceof ItemDocument) {
			System.out
					.println("The French label for entity Q8 is " + ((ItemDocument) q8).getLabels().get("fr").getText()
		...
		System.out.println("The Qid of Terry Pratchett is " + edPratchett.getEntityId().getId());
		...
		for (Entry<String, EntityDocument> entry : results.entrySet()) {
			System.out.println("Successfully retrieved data for page entitled \"" + entry.getKey() + "\": "
