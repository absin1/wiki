...
public class Helper {
	...
	/**
	 * Enum to say which dumps should be downloaded and processed. Used as possible
	 * values of {@link ExampleHelpers#DUMP_FILE_MODE}.
	 */
	...
	/**
	 * Timeout to abort processing after a short while or 0 to disable timeout. If
	 * set, then the processing will cleanly exit after about this many seconds, as
	 * if the dump file would have ended there. This is useful for testing (and in
	 * particular better than just aborting the program) since it allows for final
	 * processing and proper closing to happen without having to wait for a whole
	 * dump file to process.
	 */
	public static final int TIMEOUT_SEC = 0;

	/**
	 * Identifier of the dump file that was processed last. This can be used to name
	 * files generated while processing a dump file.
	 */
	 ...
	 * By default, the most recent JSON dump will be used. In offline mode, only the
	 * most recent previously downloaded file is considered.
	 *
	...
	public static void processEntitiesFromWikidataDump(EntityDocumentProcessor entityDocumentProcessor) {
		...
		try {
			...
			case ALL_REVS:
			case CURRENT_REVS:
				dumpFile = dumpProcessingController.getMostRecentDump(DumpContentType.FULL);
				break;
			case ALL_REVS_WITH_DAILIES:
			case CURRENT_REVS_WITH_DAILIES:
				MwDumpFile fullDumpFile = dumpProcessingController.getMostRecentDump(DumpContentType.FULL);
				MwDumpFile incrDumpFile = dumpProcessingController.getMostRecentDump(DumpContentType.DAILY);
				lastDumpFileName = fullDumpFile.getProjectName() + "-" + incrDumpFile.getDateStamp() + "."
			...
			case JSON:
				...
				break;
			case JUST_ONE_DAILY_FOR_TEST:
				dumpFile = dumpProcessingController.getMostRecentDump(DumpContentType.DAILY);
				break;
			default:
			...
			if (dumpFile != null) {
				lastDumpFileName = dumpFile.getProjectName() + "-" + dumpFile.getDateStamp();
	 ...
	 * output directory ({@link ExampleHelpers#EXAMPLE_OUTPUT_DIRECTORY}). Any file
	 * of this name that exists already will be replaced. The caller is responsible
	 * for eventually closing the stream.
	 *
	 * @param filename
	...
	/**
	 * Returns the name of the dump file that was last processed. This can be used
	 * to name files generated from this dump. The result might be the empty string
	 * if no file has been processed yet.
	 */
	 ...
	 * @throws IOException
	 *             if it was not possible to create a directory at the given path
	 */
